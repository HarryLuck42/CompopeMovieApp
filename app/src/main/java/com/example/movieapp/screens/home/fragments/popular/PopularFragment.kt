...
@Composable
fun PopularFragment(viewModel: HomeViewModel, navController: NavController) {
    Surface(
        Modifier
            .fillMaxWidth()
            .fillMaxHeight()
        ...
        val movieData =
            if (viewModel.keyword.value.isNotEmpty() && !viewModel.openSearch.value) produceState<DataOrException<MovieResponse, Boolean, Exception>>(
                ...
                value = viewModel.getAllMovies(Constants.POPULAR_PATH, "1")
            }.value

        if (movieData.loading == true) {
            CustomLoading()
        } else {
            if (movieData.data?.results?.isEmpty() != false || movieData.data?.results == null) {
                EmptyLayout()
            } else {
